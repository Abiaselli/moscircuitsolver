#!/usr/bin/env python3
# mosfet11.py  —  WD'08 neuron + live plotting + sliders
...
class NMOS(Device):
    ...
    def Id_expr(self, node_to_sym):
        ...
        return Id       # positive leaves drain
    def draw(self, C, pos):
...
def pmos_Id_leavingD_smooth(Vsg, Vsd, k, Vt, lam):
    ...
    Id_lin = k*( Vov*Vsd - sp.Rational(1,2)*Vsd**2 ); Id_lin = relu_smooth(Id_lin)
    ...
    def solve_dc(self):
        ...
        for dev in self.devices:
            if isinstance(dev, (Resistor, CurrentSourceDC)):
                add(dev.kcl_contribs(self.nodes, node_map))
        for dev in self.devices:
        ...
        def _try(g):
            ...
            for i,n in enumerate(unknown_nodes):
                v=float(root[i]); out[n]= v if isfinite(v) else 0.0
            return out
        try:
            num=_try(guess)
        except Exception:
            try:
                g2=[(gv if isfinite(gv) else 0.0) + (0.05*(i%3-1)) for i,gv in enumerate(guess)]
                num=_try(g2)
            except Exception:
                try:
                    vdd=float(self.nodes["VDD"]["value"]); num=_try([0.5*vdd for _ in unknown_nodes])
                except Exception:
                    return {}, {"kcl_sym":{n:pretty(eqs[i]) for i,n in enumerate(unknown_nodes)},"kcl_num":{}}, eqs
        self._snapshot_voltages(num)
        return num, {"kcl_sym":{n:pretty(sp.Eq(kcl[n],0)) for n in unknown_nodes},"kcl_num":{}}, eqs
    ...
    def solve_transient_step(self, dt):
        ...
        def add(d):
            for node,cur in d.items():
                if node in kcl: kcl[node] += cur
        for dev in self.devices:
            if isinstance(dev,(Resistor, CurrentSourceDC, CrossConductance, PolyCurrentToGND, ComparatorToCurrent)):
                add(dev.kcl_contribs(self.nodes, node_map))
        for dev in self.devices:
        ...
        def _try(g):
            ...
            out={}
            for i,n in enumerate(unknown_nodes):
                v=float(root[i]); out[n]= v if isfinite(v) else prev[n]
            return out
        try:
            num=_try(guess)
        except Exception:
            try:
                g2=[g + (0.02*(i%3-1)) for i,g in enumerate(guess)]
                num=_try(g2)
            except Exception:
                return None
        self.t += dt
        self._snapshot_voltages(num)
        return num, {"kcl_sym":{n:pretty(sp.Eq(kcl[n],0)) for n in unknown_nodes}}, eqs

    # Export
    def export_netlist(self, path):
        lines=["* Auto-generated netlist (mosfet11)"]
        vdd=self.nodes["VDD"]["value"]; lines.append(f"VDD VDD 0 {vdd}")
        model_cards=set()
        for d in self.devices:
...
class PlotPanel(tk.Toplevel):
    def __init__(self, master, title="V(t), U(t)"):
        super().__init__(master); self.title(title); self.configure(bg="#151515")
        self.fig = Figure(figsize=(6.6, 3.6), dpi=100)
        self.ax = self.fig.add_subplot(111)
        self.ax.set_xlabel("t (s)"); self.ax.set_ylabel("V, U"); self.ax.grid(True, alpha=0.3)
        self.lV, = self.ax.plot([], [], label="V")
        self.lU, = self.ax.plot([], [], label="U")
    ...
    def _start(self,e): self._ox,self._oy=e.x,e.y
    def _move(self,e): self.geometry(f"+{self.winfo_x()+e.x-self._ox}+{self.winfo_y()+e.y-self._oy}")
    def update(self, t, V, U):
        self.lV.set_data(t, V); self.lU.set_data(t, U)
        if len(t)>1:
            self.ax.set_xlim(t[0], t[-1])
            ymin=min(min(V),min(U)); ymax=max(max(V),max(U))
...
class Editor(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("MOS Equation Lab — mosfet11")
        ...
        # neuron param vars (WD'08)
        self.var_Vth = tk.DoubleVar(value=1.70)
        self.var_Vc  = tk.DoubleVar(value=0.60)
        self.var_D   = tk.DoubleVar(value=0.05)
        self.var_k1  = tk.DoubleVar(value=2e-4)
        self.var_k4  = tk.DoubleVar(value=2e-4)
        self.var_k6  = tk.DoubleVar(value=2e-4)
        self.var_k7  = tk.DoubleVar(value=2e-5)
        self.var_Cv  = tk.DoubleVar(value=0.1e-12)
        self.var_Cu  = tk.DoubleVar(value=1.0e-12)
        self.var_Rin = tk.DoubleVar(value=100e3)
        ...
        # traces
        self.trace_t=[]; self.trace_V=[]; self.trace_U=[]
        self.plot = PlotPanel(self, "V(t), U(t)"); self.plot.geometry("+300+540")

        # load WD’08 on startup
        self.add_template_wijekoon2008()

    # ---------- UI build ----------
    def _build_left(self):
        sidebar = ScrollableFrame(self, width=370, height=760, bg="#171717")
        ...
        tk.Button(p, text="Reset plot", command=self.reset_plot).pack(fill="x", pady=2)

        ttk.Separator(p, orient="horizontal").pack(fill="x", pady=8)
        tk.Label(p, text="Neuron (Wijekoon–Dudek) Params", fg="#ddd", bg="#171717", font=("Consolas", 11, "bold")).pack(anchor="w")

        def spin(lbl, var, frm, to, inc, cb, unit=""):
            row=tk.Frame(p,bg="#171717"); row.pack(fill="x", pady=1)
            tk.Label(row, text=lbl, fg="#ddd", bg="#171717").pack(side="left")
            sb=tk.Spinbox(row, textvariable=var, from_=frm, to=to, increment=inc, width=12,
                          font=("Consolas",10), command=cb)
            sb.pack(side="left", padx=4)
            if unit: tk.Label(row, text=unit, fg="#888", bg="#171717").pack(side="left")

        # spike/reset
        spin("Vth", self.var_Vth, 0.0, 5.0, 0.01, self.on_param_change)
        spin("Vc",  self.var_Vc,  -1.0, 5.0, 0.01, self.on_param_change)
        spin("D",   self.var_D,    0.0, 1.0, 0.005, self.on_param_change)

        ttk.Separator(p, orient="horizontal").pack(fill="x", pady=6)
        # device strengths
        spin("k1 (M1–M3)", self.var_k1, 1e-6, 1e-3, 1e-6, self.on_param_change)
        spin("k4 (M4)",    self.var_k4, 1e-6, 1e-3, 1e-6, self.on_param_change)
        spin("k6 (M6)",    self.var_k6, 1e-6, 1e-3, 1e-6, self.on_param_change)
        spin("k7 (M7)",    self.var_k7, 1e-6, 1e-3, 1e-6, self.on_param_change)
        # caps & Rin
        spin("Cv (F)", self.var_Cv, 1e-15, 1e-9, 1e-15, self.on_param_change)
        spin("Cu (F)", self.var_Cu, 1e-15, 1e-9, 1e-15, self.on_param_change)
        spin("Rin (Ω)", self.var_Rin, 1e3, 5e6, 1e3, self.on_param_change)

        tk.Button(p, text="Reset params", command=self.reset_params).pack(fill="x", pady=6)
        ...
        self.ins_text = tk.Text(self.inspector, width=32, height=16, bg="#0d0d0d", fg="#ccc",
                                insertbackground="#ccc", font=("Consolas", 10), bd=0, highlightthickness=0)
        self.ins_text.pack(fill="both", expand=True)

    def _build_canvas(self):
        self.canvas = tk.Canvas(self, width=880, height=780, bg="#0d0d0d", highlightthickness=0)
    ...
    def clear_circuit(self):
        ...
        self.circ.devices = []; self.circ.t = 0.0; self.circ.prev_node_voltages = {}
        self.selected_node = None; self.selected_dev = None
        self.reset_plot()
        self.refresh()
    ...
    def reset_plot(self):
        self.trace_t=[]; self.trace_V=[]; self.trace_U=[]
        self.plot.update(self.trace_t, self.trace_V, self.trace_U)

    def _append_trace_from_sol(self, sol):
        t=self.circ.t
        vkey = "V" if "V" in sol else ("v" if "v" in sol else None)
        ukey = "U" if "U" in sol else ("u" if "u" in sol else None)
        if vkey and ukey:
            self.trace_t.append(t); self.trace_V.append(sol[vkey]); self.trace_U.append(sol[ukey])
            self.plot.update(self.trace_t, self.trace_V, self.trace_U)
    ...
    def transient_step(self):
        ...
        ukey = "U" if "U" in sol else ("u" if "u" in sol else None)
        if vkey and ukey:
            Vth=float(self.var_Vth.get()); Vc=float(self.var_Vc.get()); D=float(self.var_D.get())
            if sol[vkey] >= Vth:
                self.circ.prev_node_voltages[vkey] = Vc
                self.circ.prev_node_voltages[ukey] = sol[ukey] + D
                sol[vkey] = Vc; sol[ukey] = sol[ukey] + D
        ...
        # add to plot
        self._append_trace_from_sol(sol)
        self.refresh()

    def transient_run(self):
        if self.running: return
        self.running=True; self.reset_plot()
    ...
    def on_param_change(self):
        # apply UI params onto the current WD’08 device instances by name
        # Spike/reset params stored directly
        # Devices:
        dev = {d.name: d for d in self.circ.devices}
        # Capacitors
        if "Cv" in dev and isinstance(dev["Cv"], Capacitor): dev["Cv"].C = float(self.var_Cv.get())
        if "Cu" in dev and isinstance(dev["Cu"], Capacitor): dev["Cu"].C = float(self.var_Cu.get())
        # Rin
        if "Rin" in dev and isinstance(dev["Rin"], Resistor): dev["Rin"].R = float(self.var_Rin.get())
        # k's
        k1=float(self.var_k1.get()); k4=float(self.var_k4.get()); k6=float(self.var_k6.get()); k7=float(self.var_k7.get())
        for name in ("M1","M2","M3"):
            if name in dev and isinstance(dev[name], NMOS): dev[name].k = k1
        if "M4" in dev and isinstance(dev["M4"], NMOS): dev["M4"].k = k4
        if "M6" in dev and isinstance(dev["M6"], NMOS): dev["M6"].k = k6
        if "M7ref" in dev and isinstance(dev["M7ref"], NMOS): dev["M7ref"].k = k7
        if "M7" in dev and isinstance(dev["M7"], NMOS): dev["M7"].k = k7
        # spike/reset vars live in Tk vars; used during transient
        self.refresh()

    def reset_params(self):
        self.var_Vth.set(1.70); self.var_Vc.set(0.60); self.var_D.set(0.05)
        self.var_k1.set(2e-4); self.var_k4.set(2e-4); self.var_k6.set(2e-4); self.var_k7.set(2e-5)
        self.var_Cv.set(0.1e-12); self.var_Cu.set(1.0e-12); self.var_Rin.set(100e3)
    ...
    def add_template_wijekoon2008(self):
        ...
        # caps
        c.devices += [Capacitor("Cv","V","GND", self.var_Cv.get()),
                      Capacitor("Cu","U","GND", self.var_Cu.get())]
        # small bias divider
        c.devices += [Resistor("Rvb1","VDD","VB",200e3), Resistor("Rvb2","VB","GND",200e3)]
        # M1–M3 positive feedback/mirror
        Vt1,lam1 = 0.5,0.01
        c.devices += [NMOS("M1","B","V","GND", self.var_k1.get(),Vt1,lam1),
                      NMOS("M2","B","B","GND", self.var_k1.get(),Vt1,lam1),
                      NMOS("M3","V","B","GND", self.var_k1.get(),Vt1,lam1)]
        # M4 leak via U
        c.devices.append(NMOS("M4","V","U","GND", self.var_k4.get(), 0.5, 0.0))
        # M6 sink at U
        c.devices.append(NMOS("M6","U","U","GND", self.var_k6.get(), 0.5, 0.0))
        # M7 scaled mirror from V to U
        c.devices += [NMOS("M7ref","B","V","GND", self.var_k7.get(), 0.5, 0.01),
                      NMOS("M7","U","B","GND",    self.var_k7.get(), 0.5, 0.01)]
        # input resistor
        c.devices.append(Resistor("Rin","IN","V", self.var_Rin.get()))
        # tiny linear conductance to aid stability
        c.devices.append(NMOS("Ml","V","VB","GND", 1e-5, 0.3, 0.0))
        # reset params object (referenced by transient)
        self.izh = {"Vth":self.var_Vth.get(), "Vc":self.var_Vc.get(), "D":self.var_D.get()}
        c.prev_node_voltages.update({"V":0.3,"U":0.2})
        self.selected_node="V"
        self.panel_dev.set("Wijekoon–Dudek 2008 neuron loaded. Adjust sliders, then Step/Run to see V(t),U(t).")
        self.reset_plot()
        self.refresh()
    ...
    def add_template_izhikevich(self):
        ...
        self.izh = {"Vth":30.0,"Vc":-65.0,"D":8.0}
        self.selected_node="v"; self.panel_dev.set("Ideal Izhikevich template."); self.reset_plot(); self.refresh()
    ...
    def add_template_izhikevich_nmos(self):
        ...
        c.prev_node_voltages.update({"v":-65.0,"u":-13.0})
        self.izh = {"Vth":30.0,"Vc":-65.0,"D":8.0}
        self.selected_node="v"; self.panel_dev.set("NMOS-only dv/dt template."); self.reset_plot(); self.refresh()
