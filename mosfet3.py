...
class NMOS(Device):
    ...
    def kcl_contribs(self, node_syms, node_to_sym):
        ...
        return {}
    def draw(self, C, nodes_pos):
        dx, dy = nodes_pos[self.d]; gx, gy = nodes_pos[self.g]; sx, sy = nodes_pos[self.s]
        # little box at centroid of d-g-s
...
class Circuit:
    ...
    # ----- DC solver (multi-device, region search)
    def solve_dc(self):
...
class Editor(tk.Tk):
    ...
    def add_dev(self, kind):
        ...
        if kind == "R":
            self.circ.devices.append(Resistor(f"R{nid}", "VDD", "N1", 10000))
        elif kind == "NMOS":
            ...
            self.circ.devices.append(CurrentSourceDC(f"I{nid}", "VDD", "N1", 1e-3))
        elif kind == "C":
    ...
    # ---------- canvas interactions
    def nodes_pos(self):
        return {n:(inf["x"], inf["y"]) for n, inf in self.circ.nodes.items()}
